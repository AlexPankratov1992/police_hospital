...
class Account extends CI_Controller
{
  ...
  public function signup()
  {
    ...
    if ($this->input->post()) {
      $this->form_validation->set_rules(array(
        ...
        array('field' => 'fees', 'label' => 'Fees', 'rules' => '',),
   ...
   *
   */
  public function update_prescription_content()
  {
    if ($this->bitauth->get_users() && !$this->bitauth->logged_in()) {
      $this->session->set_userdata('redir', current_url());
      redirect('account/login');
    ...
    if ($this->input->post()) {
      $updated = $this->input->post();
      $this->load->model('prescription_data');
      unset($updated['submit']);
      $this->prescription_data->upddata($updated);

      if ($this->form_validation->run() == TRUE) {
        unset($_POST['submit']);
        $this->bitauth->add_group($this->input->post());
        echo '<script>alert("Groups added successfully");</script>';
        redirect('account/update_prescription_content');
      }
    }
    ...
    $data['title'] = 'Update Prescription Content';
    $data['roles'] = $this->bitauth->get_roles();
    $data['users'] = $users;
    $data['p_data'] = $prescription_data[1];
    $path = 'account/update_prescription_content';
