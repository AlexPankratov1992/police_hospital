...
class Drug extends CI_Controller
{
  ...
  public function index($limit = 15, $page = 1)
  {
    ...
    $data['categories'] = $this->drugs->get_categories_list();
    $usage = $this->drugs->drug_usage_count();
    $usage_count = array();
    foreach ($usage as $key => $value) {
      $usage_count[$value->drug_id] = $value->count;
    }
    ...
    if ($search) {
      ...
      $name = $search['name'];
      $no_of_item = $search['no_of_item'];
      $times = $search['times'];
      $slot = $search['slot'];
      $data['drugs'] = $this->drugs->search_drugs($category, $name, $no_of_item, $times, $slot);

      // ============== [ end   advance search filters ] ============
    } else {
      $data['drugs'] = $this->drugs->getlist();
    }
   ...
   * drug::quickAddDrug()
   */
  public function quickAddDrug(/*$q=''*/)
  {
    if (!$this->bitauth->logged_in()) {
      $this->session->set_userdata('redir', current_url());
      redirect('account/login');
    ...
    if ($this->input->post()) {
      $this->load->model('drugs');
      $drugName = $this->input->post('q');
      $this->drugs->drug_name_en = $drugName;
      $this->drugs->dosage = '';
      $this->drugs->dosage_mg = '';
      $this->drugs->dosage_ml = '';
      $this->drugs->price = 0;
      $this->drugs->num = 0;
      $this->drugs->category = "Default";
      $this->drugs->memo = $drugName;
      $this->drugs->no_of_item = 3;
      if (strpos($drugName, 'drop') !== false)      $this->drugs->times = "L/A x 3";
      else $this->drugs->times = "3 times";
      $this->drugs->slot = "After Food";
      $this->drugs->save();
      echo "success";
      return TRUE;
    }
  ...
  public function search(/*$q=''*/)
  {
    ...
    $suggestion_data = $this->db->query("SELECT d.drug_name_en FROM drug_patient dp INNER JOIN drugs d on dp.drug_id=d.drug_id GROUP BY dp.drug_id, d.drug_name_en ORDER BY COUNT(dp.drug_id) DESC limit 8")->result();
    $drug_suggestions = array();
    foreach ($suggestion_data as $key => $value) {
      $drug_suggestions[$key] = $value->drug_name_en;
    }
    $data['drug_suggestions'] = $drug_suggestions;
    // echo "<pre>";
    // print_r($data);
    // exit;
    ...
    $this->load->view('drug/search', $data);
  }
  ...
  public function assign()
  {
    ...
    if ($this->input->post()) {
      ...
      if ($this->form_validation->run() == TRUE) {
        ...
        $epoch = $this->drug_patient->assign_date;
        ...
        echo '<tr id="dpi' . $this->drug_patient->drug_patient_id . '"><td class="id"></td>' .
          ...
          // '<td>' . $this->drug_patient->type . '</td>' .
          '<td>' . $this->drugs->drug_name_en . '</td>' .
          '<!-- td>' . $this->drugs->drug_name_fa . '</td -->' .
          '<td>' . $this->drug_patient->times . '</td>' .
          '<td>' . $this->drug_patient->slot . '</td>' .
          // '<td>' . $this->drug_patient->ofc . '</td>' .
          // '<td>' . $this->drug_patient->weight . '</td>' .
          // '<td>' . $this->drug_patient->height . '</td>' .
          '<!-- td>' . $this->drugs->price . '</td -->' .
          '<td>' . $this->drug_patient->no_of_item . '</td>' .
          '<<!-- td>' . $this->drug_patient->total_cost . '</td -->' .
          '<td class="actions">' . anchor('#', 'Delete ', array('dpi' => $this->drug_patient->drug_patient_id, 'csrf_test_name' => $this->security->get_csrf_hash(), 'di' => $this->drug_patient->drug_id, 'pi' => $this->drug_patient->patient_id, 'tc' => $this->drug_patient->total_cost, 'action' => 'delete'));
  ...
  public function payment($drug_patient_id)
  {
    ...
    if ($this->input->post()) {
      ...
      if ($this->form_validation->run() == TRUE && $this->input->post('drug_patient_id') == $drug_patient_id) {
        ...
        if (
          $this->drug_patient->drug_id == $this->input->post('drug_id') &&
