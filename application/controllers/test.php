...
class Test extends CI_Controller
{
  ...
  public function quickAddLab(/*$q=''*/)
  {
    ...
    if ($this->input->post()) {
      $this->load->model('lab');
      $labName = $this->input->post('q');
      $this->lab->test_name_en = $labName;
      $this->lab->price = 0;
      $this->lab->category = "Default";
      $this->lab->memo = $labName;
      $this->lab->save();
      echo "success";
      return TRUE;
    }
  ...
  public function assign()
  {
    ...
    if ($this->input->post()) {
      ...
      if ($this->form_validation->run() == TRUE) {
        ...
        $this->lab_patient->no_of_item = 0;
        $this->lab_patient->total_cost = 0;
        ...
        $epoch = $this->lab_patient->assign_date;
        ...
        echo '<tr id="dpi' . $this->lab_patient->lab_patient_id . '"><td class="id"></td>' .
          ...
          '<td class="actions">' . anchor('#', 'Delete ', array('dpi' => $this->lab_patient->lab_patient_id,  'csrf_test_name' => $this->security->get_csrf_hash(), 'di' => $this->lab_patient->test_id, 'pi' => $this->lab_patient->patient_id, 'tc' => $this->lab_patient->total_cost, 'action' => 'delete'));
  ...
  public function payment($lab_patient_id)
  {
    ...
    if ($this->input->post()) {
      ...
      if ($this->form_validation->run() == TRUE && $this->input->post('lab_patient_id') == $lab_patient_id) {
        ...
        if (
          $this->lab_patient->test_id == $this->input->post('test_id') &&
