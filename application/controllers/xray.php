...
class Xray extends CI_Controller
{
   ...
   * xray::quickAddXray()
   */
  public function quickAddXray(/*$q=''*/)
  {
    if (!$this->bitauth->logged_in()) {
      $this->session->set_userdata('redir', current_url());
      redirect('account/login');
    ...
    if ($this->input->post()) {
      $this->load->model('xrays');
      $xrayName = $this->input->post('q');
      $this->xrays->xray_name_en = $xrayName;
      $this->xrays->price = 0;
      $this->xrays->category = "Default";
      $this->xrays->memo = $xrayName;
      $this->xrays->save();
      echo "success";
      return TRUE;
    }
  ...
  public function assign()
  {
    ...
    if ($this->input->post()) {
      ...
      if ($this->form_validation->run() == TRUE) {
        ...
        $this->xray_patient->no_of_item = 1;
        $this->xray_patient->total_cost = 0;        
        $this->xray_patient->save();
        $this->load->model('xrays');
        $this->xrays->load($this->xray_patient->xray_id);
        $epoch = $this->xray_patient->assign_date;
        ...
        echo '<tr id="dpi' . $this->xray_patient->xray_patient_id . '"><td class="id"></td>' .
          ...
          '<td class="actions">' . anchor('#', 'Delete ', array('dpi' => $this->xray_patient->xray_patient_id, 'csrf_test_name' => $this->security->get_csrf_hash(),  'di' => $this->xray_patient->xray_id, 'pi' => $this->xray_patient->patient_id, 'tc' => $this->xray_patient->total_cost, 'action' => 'delete'));
  ...
  public function payment($xray_patient_id)
  {
    ...
    if ($this->input->post()) {
      ...
      if ($this->form_validation->run() == TRUE && $this->input->post('xray_patient_id') == $xray_patient_id) {
        ...
        if (
          $this->xray_patient->xray_id == $this->input->post('xray_id') &&
  ...
  public function details($xray_patient_id)
  {
    ...
    if ($this->input->post()) {
      ...
      if ($this->form_validation->run() == TRUE) {
        ...
        {
          $path = 'uploads/patient/' . $xray_patient_id . '/xray/';
