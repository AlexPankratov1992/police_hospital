...
class Patient_doctor extends MY_Model
{
  ...
  function get_token_no($current_session_id)
  {
    // get today date 
    // $today = date("Y-m-d");
    $query = $this->db->query("select count(*) as count from patient_doctor pd where session_id=" . $current_session_id . " group by pd.session_id");
    $row = $query->row();
    // echo "<pre>";
    // print_r($row);
    // exit;
