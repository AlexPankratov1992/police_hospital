    ...
    <fieldset>
      ...
      <div>
        ...
        <div class="form-group" title="Select all groups you would like the user to be in.">
          <div class="col-md-12"><label for="groups[]">Groups:</label><?php echo form_multiselect('groups[]', $groups, set_value('groups[]', $user->groups),"class='form-control' id='groups'");?></div>
        </div>
        <div class="form-group" style="display: none;" id="fee_tab">
            <div class="col-md-12">
                <input min="0" type="number" value="<?php echo $user->fees; ?>" name='fees' id='fees' value="" class='form-control' placeholder='Doctor Fees' title='Doctor Fees' required />
            </div>
        </div>
        <?php }if(!$this->bitauth->is_admin()){?>
...
<script>
    $(document).ready(function(){
        var roles = $('#groups').val();
        if(jQuery.inArray('3', roles) != -1){
            $("#fee_tab").css('display','block');
        }else{
            $("#fee_tab").css('display','none');
        }
