  ...
  <script>
    $(document).ready(function() {
      $('#testResult a').on('click', function(e) {
        ...
        $('#lab_patient_doctor_id').val($('#fk_patient_doctor_id').text());
        ...
        $.post($('#addTestForm').attr('action'), $('#addTestForm').serialize(), function(data) {
          if (data != '') {
            ...
            if (trWithId.last().html()) {
              ...
              $('#labGroup tbody').append('<tr class="test_unpaid text-danger"><td></td><td></td><td></td><td></td><td><!-- Unpaid: --></td><td id="test_unpaid"></td><td></td></tr>');
            ...
            Swal.fire({
              position: 'center',
              icon: 'success',
              title: 'Test has been assigned to the patient successfully',
    ...
    document.querySelector("#quickAddLab").style.display = "none";
  </script>
  </div><?php
      } else {
        echo '<div class="alert alert-danger text-center"><h1>Test Not Found</h1></div><script>document.querySelector("#quickAddLab").style.display="block";</script>';
      }
        ?>