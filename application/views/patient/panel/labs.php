<div class="tab-pane" id="labs">
      ...
      if ($(this).attr('action') == 'pay') {
        $.post('<?php echo site_url("test/payment") . "/"; ?>' + dpi, 'csrf_test_name=' + csrfVal + '&lab_patient_id=' + dpi + '&test_id=' + di + '&patient_id=' + pi, function(data) {
          if (data == 'ok') {
            ...
            unpaid -= tc;
            $('#test_paid').text(paid);
            $('#test_unpaid').text(unpaid);

            if (paid > 0) $('.test_paid').removeClass('hidden');
            else $('.test_paid').addClass('hidden');
            if (unpaid > 0) $('.test_unpaid').removeClass('hidden');
            else $('.test_unpaid').addClass('hidden');
            if (paid > 0 && unpaid > 0) $('.test_tc').removeClass('hidden');
            else $('.test_tc').addClass('hidden');
  ...
  if (($this->session->userdata('ba_user_id') == $doctor->user_id || $this->bitauth->has_role('lab')) && $status_code > 1) {
    ...
    echo form_input('patient_doctor_id', '', 'id="lab_patient_doctor_id"');
    echo form_close() . '</div>';
  } ?>
        ...
        if (@$lab) {
          ...
          foreach ($lab as $test) {
            ...
            if (!($test->user_id_discharge && $test->discharge_date)) {
              ...
              echo '<td class="actions">' . anchor('#', 'Delete ', array('csrf_test_name' => $this->security->get_csrf_hash(), 'dpi' => $test->lab_patient_id, 'di' => $test->test_id, 'pi' => $test->patient_id, 'action' => 'delete',/*'onclick'=>'drugItemsAction',*/ 'tc' => $test->total_cost));
              if ($this->bitauth->has_role('receptionist')) {
                // echo anchor('#', 'Pay ', array('csrf_test_name' => $this->security->get_csrf_hash(), 'dpi' => $test->lab_patient_id, 'di' => $test->test_id, 'pi' => $test->patient_id, 'action' => 'pay',/*'onclick'=>'drugItemsAction',*/ 'tc' => $test->total_cost));
              } else {
                echo '</td>';
              }
            } else {
