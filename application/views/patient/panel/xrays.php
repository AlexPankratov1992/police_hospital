<div class="tab-pane" id="xrays">
  ...
  if (($this->session->userdata('ba_user_id') == $doctor->user_id || $this->bitauth->has_role('xray')) && $status_code > 1) {
    ...
    echo form_input('patient_doctor_id', '', 'id="xray_patient_doctor_id"');
    echo form_close() . '</div>';
  } ?>
        ...
        if (@$xrays) {
          ...
          foreach ($xrays as $xray) {
            ...
            if (!($xray->user_id_discharge && $xray->discharge_date)) {
              ...
              echo '<td class="actions">' . anchor('#', 'Delete ', array('csrf_test_name' => $this->security->get_csrf_hash(), 'dpi' => $xray->xray_patient_id, 'di' => $xray->xray_id, 'pi' => $xray->patient_id, 'action' => 'delete', 'tc' => $xray->total_cost));
              if ($this->bitauth->has_role('receptionist')) {
                // echo anchor('#', 'Pay ', array('csrf_test_name' => $this->security->get_csrf_hash(), 'dpi' => $xray->xray_patient_id, 'di' => $xray->xray_id, 'pi' => $xray->patient_id, 'action' => 'pay', 'tc' => $xray->total_cost));
              } else {
                echo '</td>';
              }
            } else {
