  ...
  <script>
    $(document).ready(function() {
      $('#xrayResult a').on('click', function(e) {
        ...
        $('#xray_patient_doctor_id').val($('#fk_patient_doctor_id').text());
        ...
        $.post($('#addXrayForm').attr('action'), $('#addXrayForm').serialize(), function(data) {
          if (data != '') {
            ...
            if (trWithId.last().html()) {
              ...
              unpaid += tc;
              $('#xray_paid').text(paid);
              $('#xray_unpaid').text(unpaid);
              $('#xray_tc').html(unpaid + paid);
              if (paid > 0) $('.xray_paid').removeClass('hidden');
              else $('.xray_paid').addClass('hidden');
              if (unpaid > 0) $('.xray_unpaid').removeClass('hidden');
              else $('.xray_unpaid').addClass('hidden');
              if (paid > 0 && unpaid > 0) $('.xray_tc').removeClass('hidden');
              else $('.xray_tc').addClass('hidden');
            } else {
              ...
              $('#xrayGroup tbody').append('<tr class="xray_unpaid text-danger"><td></td><td></td><td></td><td></td><td></td><td id="xray_unpaid"></td><td></td></tr>');
            ...
            Swal.fire({
              position: 'center',
              icon: 'success',
              title: 'Xray has been assigned to the patient successfully',
    ...
    document.querySelector("#quickAddXray").style.display = "none";
  </script>
  </div><?php
      } else {
        echo '<div class="alert alert-danger text-center"><h1>Xray Not Found</h1></div><script>document.querySelector("#quickAddXray").style.display="block";</script>';
      }
        ?>